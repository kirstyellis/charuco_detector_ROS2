	...
	: Node("charuco_detector")
{
	...
	declare_parameter("Distortion.k1", 0.0);
	declare_parameter("Distortion.k2", 0.0);
	declare_parameter("Distortion.t1", 0.0);
	declare_parameter("Distortion.t2", 0.0);
	declare_parameter("Distortion.k3", 0.0);

	declare_parameter("Intrinsic.0_0", 0.0);
	declare_parameter("Intrinsic.0_1", 0.0);
	declare_parameter("Intrinsic.0_2", 0.0);
	declare_parameter("Intrinsic.1_0", 0.0);
	declare_parameter("Intrinsic.1_1", 0.0);
	declare_parameter("Intrinsic.1_2", 0.0);
	declare_parameter("Intrinsic.2_0", 0.0);
	declare_parameter("Intrinsic.2_1", 0.0);
	declare_parameter("Intrinsic.2_2", 0.0);

	declare_parameter("image_topic", "");
	declare_parameter("camera_info_topic", "");
	declare_parameter("use_calibration_file", false);
...
void ChArUcoDetector::setupConfigurationFromParameterServer()
{
	...
	get_parameter("Distortion.k1", distortion_[0]);
	get_parameter("Distortion.k2", distortion_[1]);
	get_parameter("Distortion.t1", distortion_[2]);
	get_parameter("Distortion.t2", distortion_[3]);
	get_parameter("Distortion.k3", distortion_[4]);

	get_parameter("Intrinsic.0_0", intrinsic_[0][0]);
	get_parameter("Intrinsic.0_1", intrinsic_[0][1]);
	get_parameter("Intrinsic.0_2", intrinsic_[0][2]);
	get_parameter("Intrinsic.1_0", intrinsic_[1][0]);
	get_parameter("Intrinsic.1_1", intrinsic_[1][1]);
	get_parameter("Intrinsic.1_2", intrinsic_[1][2]);
	get_parameter("Intrinsic.2_0", intrinsic_[2][0]);
	get_parameter("Intrinsic.2_1", intrinsic_[2][1]);
	get_parameter("Intrinsic.2_2", intrinsic_[2][2]);

	get_parameter("image_topic", image_topic_);
	get_parameter("camera_info_topic", camera_info_topic_);
	get_parameter("use_calibration_file", use_calibration_file_);
	get_parameter("calibration_file", calibration_file_);
	...
	RCLCPP_WARN(
		this->get_logger(), "Using dictionary id: %d", dictionary_id_);
	if (dictionary_id_ > 0)
	...
	RCLCPP_WARN(
		this->get_logger(),
		"Creating aruco object with, square number in x: %d, square number in y: %d, square size: %.4f, marker size: %.4f",
		number_of_squares_in_x_,
		number_of_squares_in_y_,
		squares_sides_size_m_,
		markers_sides_size_m_);
...
void ChArUcoDetector::startDetection()
{
	...
	image_subscriber_ = image_transport_ptr_->subscribe(image_topic_, 10, std::bind(&ChArUcoDetector::imageCallback, this, std::placeholders::_1));
...
void ChArUcoDetector::getCameraCalibrationCoefficient(int num_dist_coefs)
{

	camera_intrinsics_matrix = cv::Mat::zeros(3, 3, CV_64F);

	camera_distortion_coefficients_matrix = cv::Mat::zeros(1, num_dist_coefs, CV_64F);

	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			camera_intrinsics_matrix.at<double>(i, j) = intrinsic_[i][j];
		}
	}
	for (int i = 0; i < num_dist_coefs; i++)
	{
		camera_distortion_coefficients_matrix.at<double>(0, i) = distortion_[i];
	}
}
void ChArUcoDetector::cameraInfoCallback(const sensor_msgs::msg::CameraInfo::ConstSharedPtr &_msg)
{
	...
	if (use_calibration_file_)
	{
		getCameraCalibrationCoefficient(num_dist_coefs);
		return;
	}
...
void ChArUcoDetector::imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr &_msg)
{
	...
	if (camera_info_)
	{
		...
		if (detectChArUcoBoard(image_grayscale, camera_intrinsics_matrix, camera_distortion_coefficients_matrix, camera_rotation, 
			camera_translation, image_results, true))
		{
...
{
	cv::Mat temp;
	cv::bilateralFilter(image_in_out_, temp, bilateral_filter_pixel_neighborhood_, bilateral_filter_sigma_color_, bilateral_filter_sigma_space_, 
...
{
	cv::Mat temp;
	cv::adaptiveThreshold(image_in_out_, temp, adaptive_threshold_max_value_, adaptive_threshold_method_, adaptive_threshold_type_, 
		adaptive_threshold_block_size_, adaptive_threshold_constant_offset_from_mean_);
	image_in_out_ = temp;
}
										 ...
										 cv::InputOutputArray _image_with_detection_results, bool _show_rejected_markers)
{
	...
	if (valid_pose)
	{
		if (_image_with_detection_results.needed())
		{
			...
			cv::aruco::drawAxis(_image_with_detection_results, _camera_intrinsics, _camera_distortion_coefficients, _camera_rotation_out, 
				_camera_translation_out, axisLength);
		}
		return true;
	}
	return false;
